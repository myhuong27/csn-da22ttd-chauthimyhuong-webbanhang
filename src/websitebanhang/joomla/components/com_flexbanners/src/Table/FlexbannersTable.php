...
class FlexbannersTable extends Table implements VersionableTableInterface
{
    ...
    public function FlexAdBanner(DatabaseDriver $db)
    {
    ...
    public function check()
    {
        ...
        // Set startdate, enddate to null if not set
        if (!$this->startdate) {
            $this->startdate = null;
        }
    ...
    public function bind($array, $ignore = [])
    {
        if (isset($array['params']) && \is_array($array['params'])) {
            ...
            if ((int) $registry->get('width', 0) < 0) {
                $this->setError(Text::sprintf('JLIB_DATABASE_ERROR_NEGATIVE_NOT_PERMITTED', Text::_('com_flexbanners_FIELD_WIDTH_LABEL')));

                return false;
            }

            if ((int) $registry->get('height', 0) < 0) {
                $this->setError(Text::sprintf('JLIB_DATABASE_ERROR_NEGATIVE_NOT_PERMITTED', Text::_('com_flexbanners_FIELD_HEIGHT_LABEL')));
    ...
    public function store($updateNulls = true)
    {
        ...
        if (empty($this->id)) {
            ...
            if ($purchaseType < 0) {
                $purchaseType = ComponentHelper::getParams('com_flexbanners')->get('purchase_type');
            }
            ...
            $oldrow = Table::getInstance('FlexbannerTable', __NAMESPACE__ . '\\', ['dbo' => $db]);
            ...
            $table = Table::getInstance('FlexbannerTable', __NAMESPACE__ . '\\', ['dbo' => $db]);
